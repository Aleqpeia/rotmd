...
class HydrophobicEnergy:
    ...
    def calculate_per_residue_sasa(
        ...
        try:
            ...
            try:
                # Load structure from PDB with custom parameters
                # freesasa.Structure() can be picky about PDB format
                ...
                # Build residue mapping from AtomGroup
                # Map atom index in freesasa to residue ID
                atom_to_resid = {}
                for atom in protein_atoms:
                    atom_to_resid[atom.index] = int(atom.resid)

                # Sum atom SASA by residue
                n_atoms = structure.nAtoms()
                for i in range(n_atoms):
                    # Get atom SASA from freesasa
                    atom_sasa = result.atomArea(i)

                    # Map to MDAnalysis atom index
                    # Freesasa uses sequential indexing 0..N-1
                    if i < len(protein_atoms):
                        resid = int(protein_atoms[i].resid)

                        if resid in residue_sasa:
                            residue_sasa[resid] += atom_sasa
                        else:
                            residue_sasa[resid] = atom_sasa
            ...
            # Return empty dict so calculation continues without SASA
            # Energy will be calculated without hydrophobic component
            return {}
