...
def plot_pmf_contour(pmf_values: np.ndarray,
    ...
    contourf = ax.contourf(np.rad2deg(psi_grid), np.rad2deg(theta_grid), pmf_values,
                          levels=levels, cmap='terrain', extend='max')

    # Contour lines
    ax.contour(np.degrees(psi_grid), np.degrees(theta_grid), pmf_values,
                        levels=levels, colors='black', linewidths=0.6, alpha=0.4)

    
    cbar = plt.colorbar(contourf, ax=ax)
    cbar.set_label('Potential of Mean Force (kcal/mol)', fontsize=12)
