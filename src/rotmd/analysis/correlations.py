...
def extract_correlation_time(
    ...
    if method == 'integral':
        # τ_c = ∫_0^T C(τ) dτ, truncated at the first non-positive lag.
        #
        # The truncation is not optional. Beyond a few correlation times the
        # ACF is pure sampling noise fluctuating about zero, and integrating
        # all N/2 lags accumulates it: on an OU series with τ=25 and 20000
        # samples the untruncated integral returns 88.5 (tail mean 0.0099 over
        # 10^4 lags contributes ~99), and merely reseeding gives −0.1. Cutting
        # at the first zero crossing is the standard remedy and is what
        # `rotmd.analysis.equilibration.statistical_inefficiency` already does.
