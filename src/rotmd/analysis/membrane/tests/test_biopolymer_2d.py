...
def test_get_positions(pos_type, inplace, select, getselection):
    ...
    pos = obj.getPositions(
        pos_type, inplace=inplace, select=select, getselection=getselection
    )
    if inplace:
...
@pytest.mark.parametrize(
    "zlim", [14]
)  # Add test for 2 and 14, to validated interanl error management of the function
def test_FilterMinFrames(
    pos_type, inplace, select, Nframes, zlim, surf_selection=surf_selection
):
    u = mda.Universe(
        MD_NOWATER_TPR,
        MD_TRAJ,
    ...
    # else:
    selected_pos = obj.FilterMinFrames(
        pos, zlim=zlim, Nframes=Nframes, control_plots=False
    )
...
def test_PolarAnalysis(surf_selection=surf_selection):
    ...
    hist_arr, pos_hist = obj.PolarAnalysis(
        select_res,
        Nframes,  # sort=[1,2,3,4,5,6,7,8,0],
        zlim=zlim,
        control_plots=False,
        plot=True,
        ax=ax,
...
def test_RgPerpvsRgsPar(
    plot, system_name, system_label, legend, surf_selection=surf_selection
):
    u = mda.Universe(MD_NOWATER_TPR, MD_TRAJ)
    obj = BioPolymer2D(u, surf_selection=surf_selection)
    rgs = obj.getRgs2D()
    ...
    if legend is True:
        if obj.system_name is None and system_label is None:
            ...
            rg_ratio = obj.RgPerpvsRgsPar(
                rgs, color, plot=plot, system_label=system_label, legend=legend
            )
...
@pytest.mark.parametrize(
    "selection1, selection2, expected_exception",
    [
        ...
        (
            ...
            "protein",
            SelectionError,
        ),  # Invalid selection2, should raise SelectionError
        (
            "protein",
            "name CA",
            SelectionError,
        ),  # Invalid selection1, should raise SelectionError
        (
            "name CA",
            "resname DOL and prop z <16 and prot z > 5",
            SelectionError,
        ),  # Both selections invalid, should raise SelectionError
    ],
)
@pytest.mark.parametrize("inplace", [True])
def test_getHbonds(selection1, selection2, expected_exception, inplace):
    ...
    if expected_exception:
        ...
        if inplace:
            ...
            assert hasattr(
                obj, "hbonds"
            )  # Ensure the results object has expected attributes
            assert hasattr(
                obj.hbonds, "hbonds"
            )  # Ensure the results object has expected attributes
        else:
            results = obj.getHbonds(
                selection1=selection1, selection2=selection2, inplace=inplace
            )
...
def test_plotHbondsPerResidues(ax):
    ...
    sorted_df = obj.plotHbondsPerResidues(
        paths_for_contour=obj.kdeanalysis.paths, ax=ax
    )
