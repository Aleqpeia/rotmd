...
class Voronoi(MembProp):
    ...
    def properties(
        ...
        heads_pos, others = self.extend_data(
            heads_pos, dimensions, self.periodicity, others=others
        )
    ...
    def map_voronoi(self, voronoi_points, voronoi_property, nbins=None, edges=None):
        ...
        if voronoi_property.shape[0] == 1:
            ...
            grid = voronoi_property[closest_seed_indices, :].reshape(
                nbins2, nbins1, voronoi_property.shape[1]
            )
    ...
    def voronoi_apl(
        ...
        self.guess_last_cs()
        no_present = [
            lipid for lipid in list(self.lipid_list) if lipid not in lipid_list
        ]
        matrices = []
        for _ in self.u.trajectory[start:final:step]:
            ...
            selection_x = (points[:, 0] > xmin - ng * grid_size) & (
                points[:, 0] < xmax + ng * grid_size
            )
            selection_y = (points[:, 1] > ymin - ng * grid_size) & (
                points[:, 1] < ymax + ng * grid_size
            )
    ...
    def voronoi_thickness(
        self, start=None, final=None, step=None, lipid_list=None, nbins=None, edges=None
    ):
        """Function to compute and map the grid APL for several frames, map them to a 2D grid and average them
        ...
        for _ in self.u.trajectory[start:final:step]:
            ...
            voronoi_dict = self.voronoi_properties(
                layer="top", lipid_list=self.lipid_list
            )
            heights = voronoi_dict["heights"]
            for lipid in no_present:
                heights[voronoi_dict["resnames"] == lipid] = np.nan

            matrix_top, _ = self.map_voronoi(
                voronoi_dict["points"],
                heights,
                nbins,
                edges,
            ...
            voronoi_dict = self.voronoi_properties(
                layer="bot", lipid_list=self.lipid_list
            )

            heights = voronoi_dict["heights"]
            for lipid in no_present:
                heights[voronoi_dict["resnames"] == lipid] = np.nan
            matrix_bot, _ = self.map_voronoi(
                voronoi_dict["points"],
    ...
    def voronoi_height(
        ...
        for _ in self.u.trajectory[start:final:step]:
            ...
            for lipid in no_present:
                heights[voronoi_dict["resnames"] == lipid] = np.nan
            matrix_height, _ = self.map_voronoi(
                voronoi_dict["points"],
                heights,
                nbins,
                edges,
    ...
    def voronoi_splay(
        ...
        for _ in self.u.trajectory[start:final:step]:
            ...
            voronoi_dict = self.voronoi_properties(
                layer=layer, lipid_list=lipid_list, splay=True
            )
    ...
    def voronoi_order(
        ...
        def get_ids(lipids):
            ...
            if n_chain1 != 0:
                ...
                if self.forcefield == "amber":
                    layer_lip1 = self.u.select_atoms(
                        f"resid {' '.join(map(str, lip_resids - 1))}"
                    )
                angles_sn1 = OrderParameters.individual_order_sn1(
                    layer_lip1, n_chain1, atoms_inv=chain_structure[1]
                )
            ...
            if n_chain2 != 0:
                ...
                angles_sn2 = OrderParameters.individual_order_sn2(
                    layer_lip1, n_chain2, atoms_inv=chain_structure[0]
                )
        ...
        for _ in self.u.trajectory[start:final:step]:
            ...
            order_matrix, _ = self.map_voronoi(
                voronoi_points=voronoi_dict["points"],
        ...
        for chain in n_chain:
            ...
            if chain != 0:
                chains_order.append(
                    np.nanmean(order[:, :, old_chain : old_chain + chain], axis=2)
                )
            old_chain += chain
    ...
    def voronoi_all_lip_order(
        ...
        for lipid in lipid_list:
            ...
            order, edges = self.voronoi_order(
                lipid,
                layer=layer,
                n_chain=n_chain,
                start=start,
