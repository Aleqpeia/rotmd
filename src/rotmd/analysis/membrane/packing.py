...
class PackingDefects(MembProp):
    ...
    def packing_defects(
        ...
        for atom in self.radii_dict.keys():
            mat_radii_dict[atom] = self.create_circle_array(
                grid_size, self.radii_dict[atom]
            )
        ...
        for lipid in self.lipid_list:
            ...
            if periodic:
                # temp = pos_ats.copy()
                pos_ats, others = self.extend_data(
                    pos_ats, dims, self.periodicity, [elements, names]
                )
                elements = others[0]
                names = others[1]
            ...
            if not height:
                ...
                indexes, matrix_temp = self.get_indexes(
                    pos_ats,
                    nbins_aug,
                    edges=[xmin_ex, xmax_ex, ymin_ex, ymax_ex],
                    matrix_height=True,
            ...
            matrix = self.add_defects(
                matrix, indexes, elements, names, lipid, mat_radii_dict
            )
        ...
        if area:
            ...
            return_dict["area"] = {"defects": area_v, "total": area_tot}
    ...
    def add_small_matrix(big_matrix, small_matrix, center_i, center_j):
        ...
        big_matrix[big_start_i:big_end_i, big_start_j:big_end_j] += small_matrix[
            small_start_i:small_end_i, small_start_j:small_end_j
        ]

        return big_matrix

    def add_defects(self, matrix, indexes, elements, names, lipid, mat_radii_dict):
    ...
    def get_highest(data, min_lenght):
        ...
        columns = [
            "index",
            "weight",
        ]  # Data expected is an np array with columns ["index", "x", "y", "z"]
    ...
    def get_indexes(self, data, bins=10, edges=None, matrix_height=False):
        ...
        if not matrix_height:
            ...
            indexes = tuple(
                np.searchsorted(edges[i], data[:, i], side="right") for i in range(2)
            )
        else:
            indexes = tuple(
                np.searchsorted(edges[i], data[:, i], side="right") for i in range(2)
    ...
    def packing_defects_stats(
        ...
        for ts in self.u.trajectory[start:final:step]:
            _, packing_dict = self.packing_defects(
                layer=layer,
                nbins=nbins,
                height=False,
                periodic=periodic,
            ...
            if area_size:
                sizes.append(
                    packing_dict["count"]["sizes"]
                    * (packing_dict["grid_size"] * packing_dict["grid_size"])
                )
            else:
                sizes.append(packing_dict["count"]["sizes"])

        results = pd.DataFrame(
            results, columns=["defects_area", "total_area", "n_defects"]
        )
    ...
    def build_polarity_dict(self):
        ...
        for key in self.polar_dict.keys():
            polarity_dict[key] = {atom: 1 for atom in self.polar_dict[key]} | {
                atom: 0.001 for atom in self.non_polar_dict[key]
            }
