...
class Cumulative2D(MembProp):
    ...
    def order_matrix(
        ...
        for ts in self.u.trajectory[start:final:step]:
            ...
            if layer == "both":
                layer_at = self.memb.select_atoms(
                    f"byres ((resname {lipid} and name {self.working_lip[lipid]['head']}))"
                )

            else:
                layer_at = self.memb.select_atoms(
                    f"byres ((resname {lipid} and name {self.working_lip[lipid]['head']}) and prop z {self.map_layers[layer]} {z_mean})"
                )
            ...
            if n_chain1 != 0:
                ...
                if self.forcefield == "amber":
                    ...
                    layer_at1 = self.u.select_atoms(
                        f"resid {' '.join(map(str, chain1_resids))}"
                    )
                angles_sn1 = OrderParameters.individual_order_sn1(
                    layer_at1, n_chain1, atoms_inv=chain_structure[1][:n_chain1]
                )
            ...
            if n_chain2 != 0:
                ...
                angles_sn2 = OrderParameters.individual_order_sn2(
                    layer_at1, n_chain2, atoms_inv=chain_structure[0][:n_chain2]
                )
            ...
            if self.periodic:
                ...
                head_pos, carbons = self.extend_data(
                    head_pos, dimensions, self.periodicity, others=carbons
                )
        ...
        matrix = np.concatenate(
            matrix, axis=0
        )  # Expect dim (n_lipids*frames, 2+n_chain1+n_chain2)

        edges = self.edges if edges is None else edges
        print(n_chain)
        if method == "numpy":
            H, edges = self.numpyhistogram2D(
                matrix[:, :2], matrix[:, 2:], n_chain, bins=nbins, edges=edges
            )
        else:
            H, edges = self.histogram2D(
                matrix[:, :2], matrix[:, 2:], n_chain, bins=nbins, edges=edges
            )
    ...
    def histogram2D(self, sample1, weights, n_chain, bins=None, edges=None):
        ...
        Ncount = tuple(
            np.searchsorted(edges[i], sample1[:, i], side="right") for i in range(2)
        )
        for i in range(2):
    ...
    def numpyhistogram2D(self, sample1, weights, n_chain, bins=10, edges=None):
        ...
        """
        if edges is None:
            edges = [[self.edges[0], self.edges[1]], [self.edges[2], self.edges[3]]]
        else:
        ...
        hist, xedges, yedges = np.histogram2d(
            sample1[:, 0], sample1[:, 1], bins=bins, range=edges
        )
        hist[hist == 0] = 1
        ...
        for chain in n_chain:
            ...
            for i in range(chain):
                temp, xedges, yedges = np.histogram2d(
                    sample1[:, 0],
                    sample1[:, 1],
    ...
    def all_lip_order(
        ...
        for key in lipid_list:
            ...
            H, edges = self.order_matrix(
                key, layer, nbins, n_chain, edges, start=start, final=final, step=step
            )
    ...
    def surface(
        ...
        if self.verbose:
            ...
            print(
                f"We will compute the surface for a membrane with these lipids {lipid_list}"
            )
        ...
        for ts in self.u.trajectory[start:final:step]:
            ...
            if layer == "both":
                ...
                final_selection_string = (
                    f"({selection_string}) and prop z {sign} {str(mean_z)}"
                )
            ...
            if splay:
                ...
                if self.forcefield == "amber":
                    ...
                    c1 = self.u.select_atoms(
                        f"({carbons1}) and (resid " + " ".join(map(str, ids - 1)) + ")"
                    )

                    c2 = self.u.select_atoms(
                        f"({carbons2}) and (resid " + " ".join(map(str, ids + 1)) + ")"
                    )
            ...
            if self.periodic:
                ...
                head_pos, others = self.extend_data(
                    head_pos, dimensions, self.periodicity, others=others
                )
    ...
    def height_matrix(
        ...
        #### Obtain data
        data = self.surface(
            lipid_list=lipid_list, layer=layer, start=start, final=final, step=step
        )

        ### Obtain weighted 2D histogram
        H_height, x_edges, y_edges = np.histogram2d(
            x=data["x"], y=data["y"], weights=data["z"], bins=nbins, range=edges
        )
        ### Obtain count 2D histogram
        H_count, x_edges, y_edges = np.histogram2d(
            x=data["x"], y=data["y"], bins=nbins, range=edges
        )

        # Replace 0 counts with 1 to avoind division by zero
        H_count[H_count == 0] = 1.0
    ...
    def splay_matrix(
        ...
        H_splay, x_edges, y_edges = np.histogram2d(
            x=data["x"], y=data["y"], weights=data["splay"], bins=nbins, range=edges
        )
        H_count, x_edges, y_edges = np.histogram2d(
            x=data["x"], y=data["y"], bins=nbins, range=edges
        )
    ...
    def thickness(
        self, nbins=None, edges=None, lipid_list=None, start=None, final=None, step=None
    ):
        ...
        matrix_up, edges = self.height_matrix(
            lipid_list,
            "top",
            edges=edges,
            start=start,
        ...
        matrix_bot, edges = self.height_matrix(
            lipid_list,
            "bot",
            edges=edges,
            start=start,
    ...
    def project_property(
        ...
        edges = (
            ...
            if edges is None
            else [edges[:2], edges[2:]]
        )

        H_property, x_edges, y_edges = np.histogram2d(
            x=data["x"], y=data["y"], weights=data["property"], bins=nbins, range=edges
        )
        H_out = H_property
        if not distribution:
            H_count, x_edges, y_edges = np.histogram2d(
