...
class BioPolymer2D:
    ...
    def __init__(
        ...
        if isinstance(obj, mda.Universe) and biopol_selection is None:
            ...
            if isinstance(biopol_selection, str):
                ...
                raise TypeError(
                    "`biopol_selection` must be a string and a valid MDAnalysis selection"
                )
        else:
        ...
        if not start is None:
            if isinstance(start, (float, int)):
                if by_frames is True:
                    ...
                    raise TypeError(
                        "`by_frames` must be boolean. Can only be True or False."
                    )
            else:
                raise TypeError(
                    "`start` must be a float, or integer if `by_frames` is True."
                )
        ...
        if not step is None:
            if isinstance(step, (float, int)):
                ...
                raise TypeError(
                    "`step` must be a float, or integer if `by_frames` is True."
                )
        ...
        if not end is None:
            if isinstance(end, (float, int)):
                ...
                raise TypeError(
                    "`end` must be a float, or integer if `by_frames` is True."
                )
        ...
        if not surf_selection is None:
            if isinstance(surf_selection, str):
                self.surf_pos = self.getPositions(
                    select=surf_selection, inplace=False, surf_is_zero=False
                ).mean(
                    axis=(0, 1)
                )  # If want mean only over atoms set axis=1
                print(" Mean position of the surface is", self.surf_pos)
            else:
                raise TypeError(
                    "`surf_selection` must be a string and a valid MDAnalysis selection"
                )
    ...
    def getPositions(
        ...
        pos = np.zeros(
            (int((self.endF - self.startF) / self.stepF), len(ag.residues), 4)
        )
        if pos_type == "all":
            pos = np.zeros(
                (
                    int((self.endF - self.startF) / self.stepF),
                    len(ag.atoms.positions),
                    4,
                )
            )
        ...
        if not self.surf_pos is None and surf_is_zero == True:
            ...
            pos[:, :, dict_axis[self.surf_axis]] -= self.surf_pos[
                dict_axis[self.surf_axis]
            ]
        if inplace:
    ...
    def getCOMs(self, inplace=True, select=None):
        ...
        print(
            "Getting center of masses from frame",
            self.startF,
            "to",
            self.endF,
            "with steps of",
    ...
    def FilterMinFrames(pos, zlim, Nframes, control_plots=False, getNframes=False):
        ...
        if Nframes is None:
            pos_masked = pos_masked[
                np.argsort(pos_masked[:, :, 3].mean(axis=1), axis=0)
            ]
            ...
            raise ValueError(
                f"There are less than {Nframes} within the {zlim} angs threshold from the surface. Set `Nframes=None` if you want to consider all the frames < zlim. Otherwise, educe `Nframes` or  increase `zlim`."
            )
        ...
        if control_plots:
            ires = [
                0,
                1,
            ]  ## If want to change defaut residue to make control plots, change here
    ...
    def PolarAnalysis(
        ...
        if self.surf_axis in ["x", "y"]:
            raise KeyError(
                "`PolarAnalysis` method is currently only working with surfaces with normal axis in 'z' direction. Future versions will include computing this method for 'x', 'y' and 'z' directions."
            )
        elif self.surf_axis is None:
            raise KeyError(
                "Must indicate the normal axis of the surfaces with `surf_axis` attribute. `PolarAnalysis` method is currently only working with surface with normal axis in 'z' direction. Future versions will include computing this method for 'x', 'y' and 'z' directions."
            )
        ...
        endF = self.endF
        print(
            f"Computing Polar Analysis from frame {fromF} (t={self.startT}ns) to {endF} (t={self.endT}ns) "
        )
        ...
        if (
            self.surf_axis == "z"
        ):  # If surf_axis if not 'z' by default, this must change slightly
        ...
        pos_selected = self.FilterMinFrames(
            pos_centered, zlim, Nframes, control_plots=control_plots
        )
        ...
        for r in range(len(pos_selected[0])):
            ...
            radii = np.arccos(
                res_pos_selected[:, 1]
                / np.linalg.norm(res_pos_selected[:, [1, 2]], axis=1)
            )
        ...
        if plot == True:
            ...
            for i in range(ordered_selected_pos.shape[1]):
                ...
                bars = ax.bar(
                    hist_arr[sort_i[i], 0],
                    hist_arr[sort_i[i], 1] * max_height * norm_max_hist,
                    width=width,
                    label="%s" % fig_label,
                    bottom=bottom,
                    color=colors[i],
    ...
    def getRgs2D(self, plot=False):
        ...
        rg_arr = np.zeros(
            (len(self.universe.trajectory[self.startF : self.endF : self.stepF]), 4)
        )
        i = 0
    ...
    def RgPerpvsRgsPar(
        ...
        if plot == "both":
            ...
            ax.plot(
                [], [], "o", c=color, label=label
            )  # Add a single label for all data points
        elif plot == "mean":
            label = "%s (%.3f)" % (system_label, rg_ratio)
        ...
        if legend:
            if not system_label is None:
                ax.legend(
                    title=r"Syst ($\langle Rg_\perp^2\rangle /\langle Rg_\parallel^2 \rangle$)",
                    fontsize=10,
                )
            else:
                raise KeyError(
                    "Must indicate the system name with `system_name` attribute to make a legend, or assign a label to the system with `system_label` parameter."
                )
        if show:
            plt.show()
        return rg_ratio
        ...
        while i < len(vertices):
            ...
            while codes[i] == 2:
                ...
                if i == len(vertices) - 1:
                    break
            one_path.append(
                [vertices[first_i_of_path, 0], vertices[first_i_of_path, 1]]
            )
    ...
    def getKDEAnalysis(
        ...
        pos_selected = self.FilterMinFrames(
            pos, zlim, Nframes, control_plots=control_plots
        )
        ## Concatenate positions of all residues
        print(pos_selected.shape)
        pos_selected_reshape = np.reshape(
            pos_selected,
            (pos_selected.shape[0] * pos_selected.shape[1], pos_selected.shape[2]),
        )
        ...
        kde_plot = sns.kdeplot(
            df0, x=axis[0], y=axis[1], fill=True, cmap="Purples", color="black", ax=ax
        )
        kde_plot = sns.kdeplot(
            df0, x=axis[0], y=axis[1], color="black", alpha=0.5, ax=ax
        )
    ...
    def KDEAnalysisSelection(
        ...
        pos_res_contour, res = self.getPositions(
            select=select_res,
            inplace=False,
            getselection=True,
            surf_is_zero=True,
        ...
        # print(pos.shape)
        if getNframes:
            all_pos_selected, n_used_frames = self.FilterMinFrames(
                pos_res_contour, zlim, Nframes, control_plots=False, getNframes=True
            )
        else:
            all_pos_selected = self.FilterMinFrames(
                pos_res_contour, zlim, Nframes, control_plots=False
            )
    ...
    def getHbonds(
        ...
        hbonds.hydrogens_sel = (
            f"({selection1_hydrogens_sel}) or ({selection2_hydrogens_sel})"
        )
        hbonds.acceptors_sel = (
            f"({selection1_acceptors_sel}) or ({selection2_acceptors_sel})"
        )
    ...
    def HbondsPerResidues(self, sorted=True, unique_res=False):
        ...
        """
        result = np.array(
            self.hbonds.hbonds[:, [2, 3]], dtype=int
        )  # Indexes of Hydrogen and acceptors starting from 0.
        ...
        resids = np.array(
            [
                self.hbonds.hbonds[:, 0],
                self.universe.atoms[result[:, 0]].resids,
                self.universe.atoms[result[:, 1]].resids,
            ],
            dtype=int,
        )
        ...
        if unique_res:
            df.drop_duplicates(
                subset=["Frame", "Hydrogens"], inplace=True, ignore_index=True
            )
            df.drop_duplicates(
                subset=["Frame", "Acceptors"], inplace=True, ignore_index=True
            )
        print(resids.shape, result.shape, "resid", "result")
        print(df)
        Acc_resids = df[
            np.isin(df["Acceptors"], self.atom_group.residues.resids)
        ].pivot_table(columns=["Acceptors"], aggfunc="size")
        H_resids = df[
            np.isin(df["Hydrogens"], self.atom_group.residues.resids)
        ].pivot_table(columns=["Hydrogens"], aggfunc="size")
        # print(H_resids,Acc_resids)
        final_count = Acc_resids.add(
            H_resids, fill_value=0
        )  # dividing by 2 is not necesarry since a a atom will be a donot and acceptor at the same time.
        print(final_count.index - 1, "final_count.index")
        df_final = pd.DataFrame(
            np.array(
                [
                    self.universe.residues[final_count.index - 1].resids,
                    self.universe.residues[final_count.index - 1].resnames,
                    final_count,
                ]
            ).T,
    ...
    def plotHbondsPerResidues(
        self,
        paths_for_contour,
        max_circle_size=160,
        colors=["C%s" % i for i in range(10)],
        top=-1,
        contour_lvls_to_plot=None,
        contour_colors=None,
        contour_ls=None,
        contour_lw=None,
        contour_alphas=None,
        ...
        pos = self.getPositions(
            select=f"resid {str_resids}", inplace=False, surf_is_zero=False
        )
        ...
        for lvl, c, i in zip(
            contour_lvls_to_plot, contour_colors, range(len(contour_lvls_to_plot))
        ):
            # self.plotPathsInLevel(paths_for_contour,lvl,color=c,ax=ax)
            self.plotPathsInLevel(
                paths_for_contour,
                lvl,
                ax=ax,
                ls=contour_ls[i],
        ...
        for i in range(pos[:, :top].shape[1]):
            ...
            norm_val = sorted_df["Count"].iloc[i] / len(
        ...
        ax.legend(
            loc="center left",
            bbox_to_anchor=(1, 0.5),  # prop={'size':22},
            title="ResName ResIDs (Hbond %)",
        )  # title_fontsize=20)
        if show:
            plt.show()
        return sorted_df
