...
class MembProp:
    ...
    def __init__(
        ...
        if lipid_list is None:  # Select only elements of the membrane
            self.memb = self.u.select_atoms(
                "all and not protein and not\
        ...
        head_atoms = set(
            [self.working_lip[lipid]["head"] for lipid in self.working_lip]
        )
        if "P" in head_atoms:
            self.all_head = self.memb.select_atoms(
                self.build_resname(self.lipid_list) + " and name P"
            )
        else:
            self.all_head = self.memb.select_atoms(
                self.build_resname_head(self.lipid_list)
            )

        self.map_layers = {"top": " > ", "bot": " < ", "up": " > ", "down": " < "}
        ...
        if verbose:
            ...
            print(
                f"We will use the following heads and charges for the following lipids.\
    ...
    def guess_last_cs(self):
        ...
        if self.forcefield == "charmm":
            ...
            for lipid in self.lipid_list:
                ...
                carbons_1 = [
                    atom for conn in structure[0] for atom in conn if "C" in atom
                ]
                carbons_2 = [
                    atom for conn in structure[0] for atom in conn if "C" in atom
                ]
    ...
    def guess_polarity(self):
        ...
        for lipid in self.lipid_list:
            ...
            if lipid == "CHL1":
                non_polar = self.memb.select_atoms(
                    f"resid {first_lipid} and not (name O3 or name H3')"
                )
            else:
                non_polar = self.memb.select_atoms(
                    f"resid {first_lipid} and (name *C3* or name H*Y or name H*X or name H*Z  or name *C2* or name H*R or name H*S or name H*T) and not (name C3 or name C31 or name HY or name HX or name HZ  or name C2 or name C21 or name HR or name HS or name HT)"
                )
    ...
    def visualize_polarity(self, lipids="all"):
        ...
        try:
            ...
            bonds = mda.topology.guessers.guess_bonds(
                self.u.atoms, self.u.atoms.positions
            )
        ...
        if not self.non_polar_dict:
            print(
                "Non polar atoms are not yet established, please set self.non_polar_dict first"
            )
            return
        else:
            nplots = len(lipids)
            ...
            count = 0
            for lipid in lipids:
                first_lipid = self.memb.select_atoms(
                    f"resid {self.first_lipids[lipid]} and resname {lipid}"
                )
                lipid_pos = first_lipid.positions
                lipid_ats = first_lipid.names
                polarity = [
                    "blue" if name in self.non_polar_dict[lipid] else "red"
                    for name in lipid_ats
                ]
    ...
    def extend_data(data, dimensions, percentage, others=None):
        ...
        dist_y = ymax - ymin
        if len(data[0]) == 2:
            left_add = data[data[:, 0] <= xmin + percentage * dist_x] + [
                dimensions[0],
                0,
            ]
            right_add = data[data[:, 0] >= xmax - percentage * dist_x] - [
                dimensions[0],
                0,
            ]
        else:
        ...
        if others is not None:
            for i in range(len(others)):
                others[i] = np.concatenate(
                    [others[i], temp_left[i], temp_right[i]], axis=0
                )

        # Extent in y
        if len(data[0]) == 2:
            up_add = data[data[:, 1] <= ymin + percentage * dist_y] + [0, dimensions[1]]
            low_add = data[data[:, 1] >= ymax - percentage * dist_y] - [
                0,
                dimensions[1],
            ]
        else:
            up_add = data[data[:, 1] <= ymin + percentage * dist_y] + [
                0,
                dimensions[1],
                0,
            ]
        ...
        for lipid in self.lipid_list:
            ...
            if self.forcefield == "charmm":
                ...
                for edge in self.connection_chains[lipid]:
                    ...
                    non_polar.append(
                        next(comp for comp in components if edge[1] in comp)
                    )  # Stores the tails
                ...
                polar = next(
                    ...
                    if self.connection_chains[lipid][0][0] in comp
                )
                ...
                if lipid != "CHL":
                    ...
                    non_polar_final = [
                        name for name in non_polar if name not in self.polar_dict[lipid]
                    ]
            ...
            if lipid != "CHL1":
                ...
                for chain, carbs in zip(chain_graphs, carbons):
                    for carbon in carbs:
                        ...
                        connected_atoms = [
                            atom for atom in connected_atoms if "C" not in atom
                        ]
    ...
    def chain_structure(self, chain):
        ...
        neighbors = [
            [c_in_node[i]] + [item for item in neighbors[i] if "C" not in item]
            for i in range(len(c_in_node))
        ]
        return neighbors
