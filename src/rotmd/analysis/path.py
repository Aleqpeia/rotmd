...
def get_path_metric_func(name):
    ...
    try:
        ...
        errmsg = (
            f'Path metric "{key}" not found. Valid selections: '
...
def get_coord_axes(path):
    ...
    if path_dimensions == 3:
        ...
        raise ValueError(
            "Path must have 2 or 3 dimensions; the first "
...
def hausdorff(P, Q):
    ...
    *P* (*Q*) is a :class:`numpy.ndarray` of :math:`N_p` (:math:`N_q`) time
    ...
    return max(directed_hausdorff(P, Q)[0], directed_hausdorff(Q, P)[0]) / np.sqrt(N_p)
...
def dist_mat_to_vec(N, i, j):
    ...
    if not (
        ...
        and isinstance(i, numbers.Integral)
    ...
    if i < 0 or j < 0 or N < 2:
        raise ValueError(
            "Matrix indices are invalid; i and j must be greater "
            "than 0 and N must be greater the 2"
        )

    if (j > i and (i > N - 1 or j > N)) or (j < i and (i > N or j > N - 1)):
        raise ValueError(
            "Matrix indices are out of range; i and j must be "
    ...
    if j > i:
        ...
        warnings.warn(
            "Column index entered (j = {:d} is smaller than row "
        ...
        raise ValueError(
            "Error in processing matrix indices; i and j must "
...
class Path(object):
    ...
    def __init__(
        self, universe, reference, select="name CA", path_select="all", ref_frame=0
    ):
    ...
    def fit_to_reference(
        ...
        aligntrj = MDAnalysis.analysis.align.AlignTraj(
            self.u_original,
            self.u_reference,
            select=self.select,
    ...
    def to_path(self, fitted=False, select=None, flat=False):
        ...
        if fitted:
            if not isinstance(self.u_fitted, MDAnalysis.Universe):
                raise TypeError(
                    "Fitted universe not found. Generate a fitted "
    ...
    *hausdorff_pair['frame']* contains a pair of frames in the first path, *i*,
    ...
    def find_hausdorff_pair(self):
        ...
        if self.nearest_neighbors["distances"] is None:
            raise NoDataError(
                "Nearest neighbors have not been calculated yet;"
    ...
    def get_nearest_neighbors(self, frames=True, distances=True):
        ...
        if frames:
            ...
            raise NoDataError(
                'Need to select Hausdorff pair "frames" or'
    ...
    def get_hausdorff_pair(self, frames=True, distance=True):
        ...
        if self.hausdorff_pair["distance"] is None:
            raise NoDataError(
                "Hausdorff pair has not been calculated yet;"
...
class PSAnalysis(object):
    ...
    def load(self):
        ...
        if not os.path.exists(self._paths_pkl):
            raise NoDataError(
                "Fitted trajectories cannot be loaded; save file"
    ...
    def plot(
        ...
        if self.D is None:
            raise ValueError("No distance data; do 'PSAnalysis.run()' first.")
        npaths = len(self.D)
        dist_matrix = self.D
        ...
        Z, dgram = self.cluster(
            method=linkage, count_sort=count_sort, distance_sort=distance_sort
        )
        ...
        colorbar(
            im, cax=ax_color, ticks=np.linspace(minDist, maxDist, 10), format="%0.1f"
        )
    ...
    def cluster(
        ...
        try:
            if dist_mat:
                dist_vec = spatial.distance.squareform(
                    dist_mat, force="tovector", checks=True
                )
            else:
    ...
    def get_num_atoms(self):
        ...
        """
        if self.natoms is None:
            raise ValueError("No path data; do 'PSAnalysis.generate_paths()' first.")
        return self.natoms
    ...
    def get_pairwise_distances(self, vectorform=False, checks=False):
        ...
        if self.D is None:
            raise ValueError("No distance data; do 'PSAnalysis.run()' first.")
        if vectorform:
            return spatial.distance.squareform(self.D, force="tovector", checks=checks)
        else:
            return self.D
